...
<liferay-ui:success key="user.registration.success"
	message="Registration Done Successfully, Please login" />
<c:choose>
	...
	<c:otherwise>
		...
		<aui:form action="<%=loginURL%>"
			...
			name="<%=formName%>" onSubmit="event.preventDefault();">
			...
			<aui:fieldset>
				...
				<div class="form-group">
					<div class="col-md-10 col-sm-6 col-xs-12 errMsg"
						style="color: red;"></div>
				</div>
				<div class="form-group">
...
AUI().use('aui-io-request','liferay-portlet-url' ,'aui-base', function(A) {
	...
	var signInBtn= A.one(".sign-in");
	var forgotPassBtn = A.one(".forgotpassBtn");
	signInBtn.on('click', function(e) {
		A.one(".errMsg").text('');
		 var resourceURL = Liferay.PortletURL.createResourceURL();
	     resourceURL.setPortletId('com_example_portlet_MedicusRegistrationModulemvcportletPortlet');
	     resourceURL.setResourceId('/login-user');
	     resourceURL.setParameter('usernName',A.one("#<portlet:namespace/>login").val());
	     resourceURL.setParameter('pwd',A.one("#<portlet:namespace/>password").val());
	     ...
	     A.io.request(resourceURL.toString(),{
				...
				success: function() {
					...
					if(response.login){
					  window.location.href="/group/medicus";
					}else{
						A.one(".errMsg").text(response.message);
	...
	forgotPassBtn.on('click', function(e) {
		A.one(".success-forgot-msg").text('');
		A.one(".error-forgot-msg").text('')
		var resourceURL = Liferay.PortletURL.createResourceURL();
	     resourceURL.setPortletId('com_example_portlet_MedicusRegistrationModulemvcportletPortlet');
	     resourceURL.setResourceId('/forgot-password');
	     resourceURL.setParameter('email',A.one("#_com_example_portlet_MedicusRegistrationModulemvcportletPortlet_email").val());
	     ...
	     A.io.request(resourceURL.toString(),{
				...
				success: function() {
					var response=this.get('responseData');
					console.log(response.sentEmail);
